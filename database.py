...
import sqlite3
import os
import re  # <-- FIXED: Added missing import for regular expressions

DB_FILE = "sentiment_analytics.db"  # <-- FIXED: Global variable declared clearly at the top


def init_db():
    """Initializes the production database schema for storing guest sentiments."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()

    # Create master log tables for reviews
    cursor.execute("""
    ...
    # Create relational table for Aspect Multi-Mapping
    cursor.execute("""
...
def save_analysis_node(data):
    """Persists an analyzed review payload directly into the relational tables."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    ...
    for aspect in data['aspects']:
        cursor.execute("""
            INSERT INTO review_aspects (review_id, aspect_name) VALUES (?, ?)
        """, (review_id, aspect))
...
def get_aggregated_metrics():
    """Fetches full analytical aggregates across the database matrix for the UI charts."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
...
def get_word_frequencies():
    """Extracts text records, cleans out stop words, and creates a density cloud list."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
...
def get_chronological_trends():
    """Aggregates processing frequency volume grouped by calendar days."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("""
